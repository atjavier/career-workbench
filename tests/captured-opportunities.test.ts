...
async function fixture() {
  const root = await mkdtemp(join(tmpdir(), "captured-opportunities-"));
...
async function migrateThrough0019(appDataRoot: string) {
  ...
  try {
    ...
    for (const migration of migrations.filter(
      (migration) => migration.id <= "0019_captured_opportunities",
    )) {
...
test("confirmation persists one immutable local revision and metadata-only audit event", async () => {
  ...
  try {
    ...
    assert.deepEqual(library.opportunities[0], {
      id: result.opportunity.id,
      revisionId: result.opportunity.revisions[0]!.id,
      title: "Product Designer",
      company: "Northstar Studio",
      location: "Makati",
      workStyle: "Hybrid",
      ...
      originalUrl: input.postingUrl,
    });
    assert.doesNotMatch(
      JSON.stringify(library),
      /copiedDescription|contentDigest|This copied description/,
    );
    const database = openDatabase(join(value.appDataRoot, "workspace.sqlite"));
    try {
      const audit = database
        .prepare(
          "SELECT action, content_hash FROM audit_events WHERE action = 'opportunity.captured'",
        )
        .get() as { action: string; content_hash: string };
      assert.equal(audit.action, "opportunity.captured");
      assert.match(audit.content_hash, /^sha256:/);
      assert.doesNotMatch(
        JSON.stringify(audit),
        /jobs\.example|Northstar|Accessible/,
      );
      assert.throws(() =>
        database
          .prepare(
            "UPDATE captured_opportunity_revisions SET title = 'Changed'",
          )
          .run(),
      );
      assert.throws(() =>
        database.prepare("DELETE FROM captured_opportunities").run(),
      );
    } finally {
      database.close();
    }
...
test("same normalized title and company makes a non-destructive local duplicate suggestion", async () => {
  ...
  try {
    ...
    const second = await confirmCapturedOpportunity({
      ...value,
      ...input,
      postingUrl: "https://jobs.example.test/other",
      title: " product designer ",
      company: " northstar studio ",
    });
    assert.deepEqual(second.probableDuplicate, {
      title: "Product Designer",
      company: "Northstar Studio",
      capturedAt: input.capturedAt,
    });
    ...
    assert.equal(
      view.opportunities.filter((item) =>
        item.originalUrl.startsWith("https://jobs.example.test/"),
      ).length,
      2,
    );
...
test("invalid confirmation leaves no partial opportunity, revision, suggestion, or audit row", async () => {
  ...
  try {
    await assert.rejects(
      confirmCapturedOpportunity({ ...value, ...input, title: "" }),
    ...
    try {
      for (const table of [
        ...
        "captured_opportunity_revisions",
        "captured_opportunity_duplicate_suggestions",
        "audit_events",
      ])
        assert.equal(
          (
...
test("confirmation accepts an @ outside HTTPS credentials and bounds serialized requirements", async () => {
  const value = await fixture();
  try {
    const saved = await confirmCapturedOpportunity({
      ...value,
      ...input,
      postingUrl: "https://jobs.example.test/search?contact=jobs@example.test",
    });
    assert.equal(
      saved.opportunity.revisions[0]?.originalUrl,
      "https://jobs.example.test/search?contact=jobs@example.test",
    );
    await assert.rejects(
      confirmCapturedOpportunity({
        ...value,
        ...input,
        requirements: Array.from({ length: 20 }, () => "x".repeat(1000)).join(
          "\n",
        ),
      }),
