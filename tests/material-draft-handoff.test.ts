...
async function fixture() {
  const appDataRoot = await mkdtemp(join(tmpdir(), "material-draft-handoff-"));
...
test("a stored draft has a bounded readable projection and one metadata-only review handoff", async () => {
  ...
  try {
    ...
    await assert.rejects(
      ...
      { code: "MATERIAL_DRAFT_HANDOFF_DUPLICATE" },
    );
    ...
    try {
      assert.equal(
        (
          db
            .prepare(
              "SELECT COUNT(*) AS count FROM material_draft_handoffs WHERE draft_id = ?",
            )
            .get(value.draftId) as { count: number }
        ).count,
        1,
      );
      assert.equal(
        (
          db
            .prepare("SELECT COUNT(*) AS count FROM material_versions")
            .get() as { count: number }
        ).count,
        0,
      );
      const audit = db
        .prepare(
          "SELECT action, content_hash FROM audit_events WHERE entity_id = ? AND action = 'resume.material_draft_handed_off'",
        )
        .get(value.draftId) as { action: string; content_hash: string };
      assert.equal(audit.action, "resume.material_draft_handed_off");
      assert.match(audit.content_hash, /^sha256:/);
      assert.doesNotMatch(JSON.stringify(audit), /Tailor|TypeScript|Portfolio/);
    } finally {
      db.close();
    }
  } finally {
  ...
  try {
    ...
    try {
      const workspaceId = (
        db
          .prepare(
            "SELECT workspace_id AS workspaceId FROM resume_workspace_drafts LIMIT 1",
          )
          .get() as { workspaceId: string }
      ).workspaceId;
      draftId = persistResumeCoachDraft({
        ...
        response: {
          ...
          sections: [
            {
              heading: "Projects",
              text: "Portfolio | Interface workflow\n- Developed an accessible TypeScript interface for researchers.",
            },
          ],
          claims: [
            {
              text: "Developed an accessible TypeScript interface for researchers.",
              evidenceIndexes: [],
          ...
          candidateClarifications: [
            {
              ...
              text: "I built an accessible TypeScript interface for researchers.",
      ...
      assert.equal(
        (
          db
            .prepare(
              "SELECT COUNT(*) AS count FROM material_claim_support WHERE claim_id IN (SELECT id FROM material_draft_claims WHERE draft_id = ?)",
            )
    ...
    assert.deepEqual(draft.claims[0]?.evidence, []);
    assert.deepEqual(draft.claims[0]?.candidateClarifications, [
      {
        itemName: "Portfolio",
        itemCategory: "project",
        category: "users_workflow",
...
test("corrupt material drafts cannot be read or handed off", async () => {
  const value = await fixture();
  const corruptId = "00000000-0000-7000-8000-000000000016";
  try {
    ...
    try {
      db.prepare(
        "INSERT INTO material_drafts (id, kind, profile_revision_id, profile_content_digest, template_source_id, template_content_digest, opportunity_revision_id, opportunity_content_digest, request_text, request_digest, content_json, content_digest, provenance_digest, created_at) VALUES (?, 'resume', ?, ?, ?, ?, NULL, NULL, 'Tailor my resume', ?, ?, ?, ?, ?)",
      ).run(
        corruptId,
        ids.profileRevision,
        hash("a"),
    ...
    await assert.rejects(
      readMaterialDraft({ appDataRoot: value.appDataRoot, draftId: corruptId }),
      { code: "MATERIAL_DRAFT_INVALID" },
    );
    await assert.rejects(
      handOffMaterialDraft({
    ...
    const verify = openDatabase(value.databasePath);
    try {
      assert.equal(
        (
          verify
...
test("an active-workspace draft read rejects a draft retained by another resume", async () => {
  ...
  try {
    assert.equal(
      (
        await readActiveWorkspaceMaterialDraft({
          appDataRoot: value.appDataRoot,
          draftId: value.draftId,
        })
      ).id,
...
test("claim support outside the draft's selected evidence rejects the entire draft", async () => {
  const value = await fixture();
  const secondEvidence = "00000000-0000-7000-8000-000000000017";
  const secondRevision = "00000000-0000-7000-8000-000000000018";
  try {
    const db = openDatabase(value.databasePath);
    try {
      db.prepare(
        "INSERT INTO evidence_records (id, created_at) VALUES (?, ?)",
      ).run(secondEvidence, "2026-08-26T00:00:00.000Z");
      db.prepare(
        "INSERT INTO evidence_revisions (id, evidence_id, revision_number, origin, source_document, source_section, factual_text, review_state, created_at, content_digest) VALUES (?, ?, 1, 'user_entered', 'Portfolio', 'Unselected case study', 'Unselected factual support', 'approved', ?, ?)",
      ).run(
        secondRevision,
        secondEvidence,
        "2026-08-26T00:00:00.000Z",
      ...
      const claim = db
        .prepare("SELECT id FROM material_draft_claims WHERE draft_id = ?")
        .get(value.draftId) as { id: string };
      db.prepare(
        "INSERT INTO material_claim_support (claim_id, evidence_revision_id) VALUES (?, ?)",
      ).run(claim.id, secondRevision);
    } finally {
      db.close();
    }
    await assert.rejects(
  ...
  try {
    ...
    try {
      assert.equal(
        (
          db
            .prepare("SELECT COUNT(*) AS count FROM material_draft_handoffs")
            .get() as { count: number }
        ).count,
        1,
      );
      assert.equal(
        (
          db
            .prepare(
              "SELECT COUNT(*) AS count FROM audit_events WHERE action = 'resume.material_draft_handed_off'",
            )
            .get() as { count: number }
        ).count,
        1,
