...
test("application shell presents Jobs first with human-facing destinations", async () => {
  const shell = await readFile(new URL("../src/app/application-shell.tsx", import.meta.url), "utf8");
  assert.match(shell, /Jobs/);
  for (const label of ["Resume", "Settings", "Review saved opportunities"]) assert.match(shell, new RegExp(label));
  for (const label of ["Applications", "Evidence Library", "Google Sheets", "Career Assistant"]) assert.doesNotMatch(shell, new RegExp(`label: "${label}"`));
...
test("placeholder destinations retain the shell, route identity, and honest status", async () => {
  const placeholder = await readFile(new URL("../src/app/[section]/page.tsx", import.meta.url), "utf8");
...
test("shell styling uses the approved premium header and visible focus token", async () => {
  ...
  assert.match(styles, /--header: #173B2D/);
  assert.match(styles, /--primary: #2F6B57/);
