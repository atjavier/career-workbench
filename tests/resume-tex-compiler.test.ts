...
for (const [kind, name] of [
  ["structured", "compileResumeDraftPdf"],
  ["raw", "compileRawTexDraftPdf"],
] as const) {
  test(`${name} preserves command, cwd, output validation and cleanup at runtime`, async () => {
    const root = await realpath(
      await mkdtemp(join(tmpdir(), "resume-tex-compiler-contract-")),
    );
    try {
      const result = await execFileAsync(
        ...
        [
          ...
          "--import",
          "tsx",
          "tests/fixtures/resume-tex-compiler-contract.mts",
          kind,
          root,
        ],
