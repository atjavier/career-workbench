...
const source = (file: string) => readFile(new URL(`../src/app/${file}`, import.meta.url), "utf8");

test("compact navigation preserves Jobs-first routes and keyboard-close focus behavior", async () => {
  const [shell, compact, styles] = await Promise.all([source("application-shell.tsx"), source("compact-navigation.tsx"), source("globals.css")]);
...
test("shared visual polish uses semantic actions, resilient cards, and disclosure", async () => {
  const [styles, jobs, applications, current, evidence, assistant, storage] = await Promise.all([
    source("globals.css"), source("job-listings.tsx"), source("applications.tsx"), source("current-base-resume.tsx"), source("evidence-review.tsx"), source("career-assistant.tsx"), source("data-storage.tsx"),
  ]);
