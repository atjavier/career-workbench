...
async function fixture() {
  ...
  await writeFile(
    source,
    "Skills: TypeScript\nBuilt a local workspace",
    "utf8",
  );
  const appDataRoot = join(root, "private");
  const baseResume = await importBaseResume({
    appDataRoot,
...
test("evidence revisions are immutable and only a current approved revision is claim eligible", async () => {
  ...
  try {
    const added = await addManualEvidence({
      appDataRoot: value.appDataRoot,
      factualText: "Built an accessible local workspace",
      sourceDocument: "Adrian interview notes",
    ...
    const approved = await approveEvidence({
      appDataRoot: value.appDataRoot,
      evidenceId: added.evidenceId,
      expectedRevisionId: added.id,
    });
    assert.equal(approved.revisionNumber, 2);
    assert.equal(
      (await listClaimEligibleEvidence({ appDataRoot: value.appDataRoot }))[0]
        .id,
      approved.id,
    );
    const edited = await editEvidence({
      appDataRoot: value.appDataRoot,
      evidenceId: added.evidenceId,
      expectedRevisionId: approved.id,
      factualText: "Built an accessible local-first workspace",
    });
...
test("extraction reads only the copied Base Resume and reject/remove retain ineligible history", async () => {
  ...
  try {
    ...
    const extracted = await extractEvidenceFromBaseResume({
      appDataRoot: value.appDataRoot,
      baseResumeId: value.baseResume.baseResume.id,
    });
    ...
    const rejected = await rejectEvidence({
      appDataRoot: value.appDataRoot,
      evidenceId: extracted[0].evidenceId,
      expectedRevisionId: extracted[0].id,
    });
    const removed = await removeEvidence({
      appDataRoot: value.appDataRoot,
      evidenceId: extracted[1].evidenceId,
      expectedRevisionId: extracted[1].id,
    });
