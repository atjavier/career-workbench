...
async function fixture() {
  const root = await mkdtemp(join(tmpdir(), "opportunity-capture-"));
...
test("creates a bounded local capture draft without inferring from the URL", () => {
  ...
  const unknown = captureOpportunityDraft({
    postingUrl: "https://company.example.test/jobs/senior-design-engineer",
    copiedDescription:
      "A deliberately plain posting without labelled company, location, work style, requirements, or posted date. It has enough copied content to validate without permitting the parser to invent anything from the address.",
    now: () => new Date("2026-08-24T12:00:00.000Z"),
  });
  assert.equal(unknown.company, "Unknown");
  assert.equal(unknown.location, "Unknown");
  assert.equal(unknown.workStyle, "Unknown");
  assert.deepEqual(unknown.requirements, ["Unknown"]);
  assert.equal(unknown.postedAt, "Unknown");
  const labelled = captureOpportunityDraft({
    postingUrl: "https://jobs.example.test/role",
    copiedDescription: `Job title: Senior Product Designer\nCompany: Northstar Studio\n\nThis is a complete copied job posting with enough detail to be reviewed safely without using facts that came from its URL or another system.`,
    now: () => new Date("2026-08-24T12:00:00.000Z"),
  });
  assert.equal(labelled.title, "Senior Product Designer");
  const proseFirst = captureOpportunityDraft({
    postingUrl: "https://jobs.example.test/role",
    copiedDescription: `About the role\nCompany: Northstar Studio\n\nThis is a complete copied job posting with enough detail to be reviewed safely without converting its introductory section heading into a job title.`,
    now: () => new Date("2026-08-24T12:00:00.000Z"),
  });
...
test("rejects URL and copied-description boundaries with safe capture errors", () => {
  ...
  for (const postingUrl of [
    "",
    "http://jobs.example.test/role",
    "https://user:pass@jobs.example.test/role",
    `https://jobs.example.test/${"a".repeat(2049)}`,
  ])
    assert.throws(() => captureOpportunityDraft({ ...valid, postingUrl }), {
      code: "OPPORTUNITY_CAPTURE_INVALID",
    });
  for (const text of [" ", "a".repeat(79), "a".repeat(200001)])
    assert.throws(
      () => captureOpportunityDraft({ ...valid, copiedDescription: text }),
      { code: "OPPORTUNITY_CAPTURE_INVALID" },
    );
  for (const posted of ["2026-02-30", "2025-02-29", "2026-13-01"])
    assert.equal(
      captureOpportunityDraft({
        ...valid,
        copiedDescription: copiedDescription.replace("2026-08-20", posted),
      }).postedAt,
      "Unknown",
    );
  assert.equal(
    captureOpportunityDraft({
      ...valid,
      copiedDescription: copiedDescription.replace("2026-08-20", "2024-02-29"),
    }).postedAt,
