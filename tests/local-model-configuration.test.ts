...
} from "../src/domain/resume-generation/local-model-configuration-commands";
      ...
      models: [
        {
          ...
          loaded_instances: loaded
            ? [{ id: "instance-1", context_length: 30000 }]
            : [],
        },
      ],
    }),
...
test("verified local-model settings create an immutable tokenless selected configuration", async () => {
  ...
  try {
    let target = "";
    let authorization = "";
    const saved = await configureLocalModel({
    ...
    assert.equal(
      ...
      JSON.stringify({ ready: true, displayLabel: "Qwen3.5-9B" }),
    );
    ...
    try {
      ...
      const row = db
        .prepare(
          "SELECT endpoint, model_identifier, display_label, secret_reference, configuration_digest FROM local_model_configuration_revisions",
        )
      ...
      assert.throws(
        () =>
          db.exec(
            "UPDATE local_model_configuration_revisions SET model_identifier = 'changed'",
          ),
        /immutable/i,
      );
    } finally {
    ...
    assert.deepEqual(
      await readLocalModelReadiness({ appDataRoot: join(root, "private") }),
      { ready: true, displayLabel: "Qwen3.5-9B" },
    );
  } finally {
    ...
    try {
      ...
      assert.equal(
        (
          db
            .prepare(
              "SELECT COUNT(*) AS count FROM local_model_configuration_revisions",
            )
            .get() as { count: number }
        ).count,
        0,
...
test("a stale selection and a failed database save leave no selected configuration", async () => {
  ...
  try {
    ...
    try {
      ...
      db.exec(
        "CREATE TRIGGER test_configuration_insert_failure BEFORE INSERT ON local_model_configuration_revisions BEGIN SELECT RAISE(ABORT, 'injected save failure'); END;",
      );
    } finally {
      db.close();
    }
    ...
    try {
      ...
      assert.equal(
        (
          verify
            .prepare(
              "SELECT current_model_configuration_revision_id FROM resume_generation_state WHERE singleton = 1",
            )
            .get() as { current_model_configuration_revision_id: string | null }
        ).current_model_configuration_revision_id,
        null,
      );
    } finally {
      verify.close();
...
test("supports configuring non-Qwen loaded models and preserves dynamic displayLabel", async () => {
  ...
  try {
        ...
        JSON.stringify({
          models: [
            ...
            },
            {
              ...
              loaded_instances: [
                { id: "inst-llama-1", config: { context_length: 16384 } },
              ],
            },
          ],
        }),
    ...
    const readinessGlm = await readLocalModelReadiness({
      appDataRoot: join(root, "private"),
    });
    assert.deepEqual(readinessGlm, {
      ready: true,
      displayLabel: "glm-5.3-flash",
    });
    ...
    const readinessLlama = await readLocalModelReadiness({
      appDataRoot: join(root, "private"),
    });
    assert.deepEqual(readinessLlama, {
      ready: true,
      displayLabel: "llama-3.2-3b",
    });
  } finally {
    await rm(root, { recursive: true, force: true });
  }
