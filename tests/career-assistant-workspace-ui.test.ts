...
test("Career Assistant has a dedicated local-first shared-shell route", async () => {
  const [page, placeholder] = await Promise.all([
    readFile(
      new URL("../src/app/career-assistant/page.tsx", import.meta.url),
      "utf8",
    ),
...
test("Career Assistant discloses the bounded local project-evidence workflow before inspection", async () => {
  ...
  assert.match(ui, /selectedFolderName/);
  assert.match(ui, /Unverified proposal/);
  assert.match(ui, /Proposal list is unavailable/);
});
...
test("Career Assistant preserves individual proposal review and recovery truthfulness", async () => {
  ...
  for (const text of [
    ...
    "partial available results",
    "Safe next action",
    "Evidence Review",
  ])
    assert.match(ui, new RegExp(text, "i"));
  assert.match(workspace, /listDocumenterProposals/);
  assert.match(workspace, /safeNextAction/);
...
test("Career Assistant stays explicit and does not add unsafe client behavior", async () => {
  ...
  assert.doesNotMatch(
    source,
    /fetch\s*\(|setInterval|setTimeout|watch\s*\(|oauth|showDirectoryPicker/i,
  );
  assert.doesNotMatch(
    source,
    /LM_STUDIO_API_TOKEN|absolutePath|prompt|raw response|audit payload/i,
  );
});
