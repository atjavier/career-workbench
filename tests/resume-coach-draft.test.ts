...
test("Coach response becomes an immutable provenance-backed draft with metadata-only audit", async () => {
  ...
  try {
    const workspace = await createResumeWorkspace({ appDataRoot: root, name: "Coach draft test" });
