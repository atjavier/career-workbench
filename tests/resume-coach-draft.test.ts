...
const ids = {
  profile: "00000000-0000-7000-8000-000000000001",
  profileRevision: "00000000-0000-7000-8000-000000000002",
  template: "00000000-0000-7000-8000-000000000003",
  evidence: "00000000-0000-7000-8000-000000000004",
  evidenceRevision: "00000000-0000-7000-8000-000000000005",
  opportunity: "00000000-0000-7000-8000-000000000006",
  opportunityRevision: "00000000-0000-7000-8000-000000000007",
};

test("Coach response becomes an immutable provenance-backed draft with metadata-only audit", async () => {
  const root = await mkdtemp(join(tmpdir(), "resume-coach-draft-"));
  const databasePath = join(root, "workspace.sqlite");
  const timestamp = "2026-08-25T00:00:00.000Z";
  try {
    const workspace = await createResumeWorkspace({
      appDataRoot: root,
      name: "Coach draft test",
    });
    ...
    try {
      ...
      db.prepare(
        "INSERT INTO evidence_revisions (id, evidence_id, revision_number, origin, source_document, source_section, factual_text, review_state, created_at, content_digest) VALUES (?, ?, 1, 'user_entered', 'Experience', 'Project', 'Built TypeScript UI', 'approved', ?, ?)",
      ).run(ids.evidenceRevision, ids.evidence, timestamp, hash("c"));
      db.prepare(
        "INSERT INTO captured_opportunities (id, duplicate_key, created_at) VALUES (?, 'frontend engineer|example', ?)",
      ).run(ids.opportunity, timestamp);
      db.prepare(
        "INSERT INTO captured_opportunity_revisions (id, opportunity_id, captured_at, original_url, copied_description, title, company, location, work_style, requirements, posted_at, content_digest, created_at) VALUES (?, ?, ?, 'https://jobs.example.test/frontend-engineer', 'Frontend Engineer role with TypeScript, accessibility, collaboration, testing, and local work. This description is intentionally long enough to be a valid local capture.', 'Frontend Engineer', 'Example', 'Manila', 'Hybrid', '[\"TypeScript\"]', '2026-08-25T00:00:00.000Z', ?, ?)",
      ).run(
        ids.opportunityRevision,
        ids.opportunity,
        timestamp,
        hash("e"),
        timestamp,
    ...
    const result = persistResumeCoachDraft({
      ...
      opportunity: {
        revisionId: ids.opportunityRevision,
        contentDigest: hash("e"),
      },
    ...
    try {
      assert.equal(
        (
          verify
            .prepare("SELECT COUNT(*) AS count FROM material_drafts")
            .get() as { count: number }
        ).count,
        1,
      );
      assert.equal(
        (
          verify
            .prepare("SELECT COUNT(*) AS count FROM material_draft_evidence")
            .get() as { count: number }
        ).count,
        1,
      );
      assert.equal(
        (
          verify
            .prepare("SELECT COUNT(*) AS count FROM material_claim_support")
            .get() as { count: number }
        ).count,
        1,
      );
      const opportunity = verify
        .prepare(
          "SELECT opportunity_revision_id AS revisionId, opportunity_content_digest AS digest FROM material_drafts WHERE id = ?",
        )
        .get(result.id) as { revisionId: string; digest: string };
      assert.equal(opportunity.revisionId, ids.opportunityRevision);
      assert.equal(opportunity.digest, hash("e"));
      const audit = verify
        .prepare(
          "SELECT action, content_hash FROM audit_events WHERE entity_id = ?",
        )
        .get(result.id) as { action: string; content_hash: string };
      assert.equal(audit.action, "resume.material_draft_created");
      assert.match(audit.content_hash, /^sha256:/);
      assert.doesNotMatch(JSON.stringify(audit), /Tailor|TypeScript/);
      assert.throws(
        () =>
          verify
            .prepare(
              "UPDATE material_drafts SET request_text = 'changed' WHERE id = ?",
            )
            .run(result.id),
        /immutable/,
      );
