...
async function seedLegacyCurrentBaseResume(
  appDataRoot: string,
  bytes: Uint8Array,
) {
  ...
  const content = await parseResumePdf(bytes);
  ...
  try {
    ...
    db.prepare(
      ...
      hash(bytes),
      bytes.byteLength,
      copied.storageLocation,
      now,
    );
    db.prepare(
      "INSERT INTO current_base_resume_drafts (id, source_id, parent_draft_id, revision_number, content_json, content_digest, created_at) VALUES (?, ?, NULL, 1, ?, ?, ?)",
    ).run(
      draftId,
      sourceId,
...
test("PDF parser accepts embedded text only and safely rejects non-PDF, empty and oversized inputs", async () => {
  const parsed = await parseResumePdf(
    textPdf("Adrian Javier built local systems"),
  );
  assert.ok(Object.values(parsed).flat().join(" ").includes("Adrian Javier"));
  await assert.rejects(parseResumePdf(new Uint8Array([1, 2, 3])), {
    code: "CURRENT_BASE_RESUME_INVALID",
  });
  await assert.rejects(
    parseResumePdf(
      new Uint8Array(
        await readFile(
          new URL(
            "./fixtures/current-base-resume/invalid.pdf",
            import.meta.url,
          ),
        ),
      ),
  ...
  await assert.rejects(
    parseResumePdf(
      new Uint8Array(
        await readFile(
          new URL(
            "./fixtures/current-base-resume/scanned.pdf",
            import.meta.url,
          ),
        ),
      ),
  ...
  await assert.rejects(
    parseResumePdf(new Uint8Array(maximumCurrentResumeBytes + 1)),
    { code: "CURRENT_BASE_RESUME_INVALID" },
  );
});
...
test("structured PDF text preserves headings and draft persistence only accepts bounded valid sections", () => {
  ...
  assert.throws(
    () => validateResumeDraftContent({ summary: ["only one section"] }),
    { code: "CURRENT_BASE_RESUME_INVALID" },
  );
  assert.throws(
    () =>
...
test("legacy Current Base Resume import is history-only and staging helpers still clean failed copies", async () => {
  const root = await mkdtemp(join(tmpdir(), "current-resume-failure-"));
  ...
  try {
    ...
    await mkdir(join(appDataRoot, "current-base-resumes", "test-id"), {
      recursive: true,
    });
    await writeFile(
      join(appDataRoot, "current-base-resumes", "test-id", "existing.pdf"),
      "existing",
    );
    await assert.rejects(
      stageCurrentBaseResume(appDataRoot, "test-id", "Resume.pdf", textPdf()),
      /EPERM|EEXIST|ENOTEMPTY|exist/i,
    );
    await assert.rejects(
      readFile(join(appDataRoot, ".import-staging", "test-id", "Resume.pdf")),
    );
  } finally {
...
test("legacy Current Base Resume write commands are blocked without creating a replacement history", async () => {
  const root = await mkdtemp(join(tmpdir(), "current-resume-"));
  const appDataRoot = join(root, "private");
  try {
    const content = {
      contact: [],
    ...
    await assert.rejects(
      resolveCurrentBaseResumeProposal({
        ...
        expectedDecisionRevisionId: createUuidV7(),
        decision: "approved",
      }),
      { code: "CURRENT_BASE_RESUME_HISTORY_ONLY" },
    );
    ...
    const state = await listCurrentBaseResume({ appDataRoot });
...
test("Current Base Resume PDF reads only the current, retained source with verified immutable bytes", async () => {
  const root = await mkdtemp(join(tmpdir(), "current-resume-preview-"));
  const appDataRoot = join(root, "private");
  const original = textPdf("Faithful original PDF");
  try {
    const imported = await seedLegacyCurrentBaseResume(appDataRoot, original);
    assert.deepEqual(
      await readCurrentBaseResumePdf({
        appDataRoot,
    ...
    await writeFile(
      join(appDataRoot, imported.storageLocation),
      textPdf("Modified retained bytes"),
    );
    assert.equal(
      await readCurrentBaseResumePdf({
        appDataRoot,
...
test("Current Base Resume PDF route returns verified inline bytes and normalizes unavailable sources", async () => {
  const root = await mkdtemp(join(tmpdir(), "current-resume-route-"));
  const priorLocalAppData = process.env.LOCALAPPDATA;
  process.env.LOCALAPPDATA = root;
  const appDataRoot = join(root, "PersonalJobDiscovery");
  const original = textPdf("Route PDF bytes");
  try {
    const imported = await seedLegacyCurrentBaseResume(appDataRoot, original);
    const response = await getCurrentBaseResumePdf(
      new Request("http://localhost/api/current-base-resume/preview"),
      { params: Promise.resolve({ sourceId: imported.sourceId }) },
    );
    ...
    for (const sourceId of [
      ...
      const unavailable = await getCurrentBaseResumePdf(
        new Request("http://localhost/api/current-base-resume/preview"),
      ...
      assert.doesNotMatch(
        ...
        /current-base-resumes|sha256|[A-Z]:\\/i,
      );
    }
    await writeFile(
      join(appDataRoot, imported.storageLocation),
      textPdf("Modified route bytes"),
    );
    const tampered = await getCurrentBaseResumePdf(
      new Request("http://localhost/api/current-base-resume/preview"),
      { params: Promise.resolve({ sourceId: imported.sourceId }) },
    );
...
test("Current Base Resume UI remains a safe retained-history component outside the active Resume page", async () => {
  ...
  assert.match(ui, /htmlFor="current-resume-pdf"/);
  assert.match(ui, /accept="application\/pdf,.pdf"/);
  assert.match(ui, /role="status"/);
  assert.match(ui, /aria-live="polite"/);
  assert.match(ui, /value="approved"/);
  assert.match(ui, /value="rejected"/);
  assert.doesNotMatch(
    ui,
  ...
  assert.match(ui, /Coach not available/);
  assert.match(ui, /Resume Coach is not available/);
  assert.match(ui, /View retained Current Base Resume versions/);
  assert.match(page, /readCandidateProfileState/);
  assert.match(page, /ResumeOnboarding/);
  assert.doesNotMatch(
    page,
    /CurrentBaseResume|sourceCount: value\.sources\.length|sourceId,/,
  );
});
...
test("faithful original-PDF preview keeps source storage private and route responses security-bound", async () => {
  const [ui, route, commands, files] = await Promise.all([
    ...
    readFile(
      new URL(
        "../src/app/api/current-base-resume/[sourceId]/pdf/route.ts",
        import.meta.url,
      ),
      "utf8",
    ),
    ...
    readFile(
      new URL("../src/files/current-base-resume.ts", import.meta.url),
      "utf8",
    ),
  ]);
  assert.match(ui, /Original, read-only Resume PDF/);
  assert.match(ui, /Open original PDF/);
  assert.match(ui, /Editing this draft does not change the original PDF/);
  assert.match(ui, /Your editable draft is still available/);
  assert.doesNotMatch(
    ui,
    /storageLocation|contentDigest|absolute path|parser diagnostics|Technical support details|Retained evidence revision identifiers|No scripts, remote assets, or cloud processing/i,
  );
  for (const header of [
    "application/pdf",
    "inline",
  ...
  assert.match(route, /The original resume PDF is unavailable/);
  assert.doesNotMatch(route, /storageLocation|contentDigest|absolute path/i);
  assert.match(commands, /readCurrentBaseResumePdf/);
  assert.match(commands, /digest\(bytes\) !== source\.contentDigest/);
  assert.match(files, /isSymbolicLink/);
  assert.match(files, /sourceStorageSegments/);
  assert.match(files, /relative\(root, path\)/);
});
