CREATE TABLE captured_opportunities (
...
CREATE TABLE captured_opportunity_duplicate_suggestions (
  id TEXT PRIMARY KEY CHECK (id GLOB '????????-????-7???-[89ab]???-????????????'),
  opportunity_id TEXT NOT NULL REFERENCES captured_opportunities(id),
  probable_duplicate_opportunity_id TEXT NOT NULL REFERENCES captured_opportunities(id),
  created_at TEXT NOT NULL CHECK (strftime('%Y-%m-%dT%H:%M:%fZ', created_at) = created_at),
  content_digest TEXT NOT NULL CHECK (length(content_digest) = 71 AND content_digest GLOB 'sha256:*' AND substr(content_digest, 8) NOT GLOB '*[^0-9a-f]*'),
  CHECK (opportunity_id <> probable_duplicate_opportunity_id), UNIQUE(opportunity_id, probable_duplicate_opportunity_id)
);
CREATE INDEX captured_opportunities_duplicate_key_index ON captured_opportunities(duplicate_key);
CREATE INDEX captured_opportunity_revisions_opportunity_index ON captured_opportunity_revisions(opportunity_id);
CREATE TRIGGER captured_opportunities_immutable_update BEFORE UPDATE ON captured_opportunities BEGIN SELECT RAISE(ABORT, 'captured opportunities are immutable'); END;
CREATE TRIGGER captured_opportunities_immutable_delete BEFORE DELETE ON captured_opportunities BEGIN SELECT RAISE(ABORT, 'captured opportunities are immutable'); END;
...
CREATE TRIGGER captured_opportunity_duplicate_suggestions_immutable_update BEFORE UPDATE ON captured_opportunity_duplicate_suggestions BEGIN SELECT RAISE(ABORT, 'captured opportunity duplicate suggestions are immutable'); END;
CREATE TRIGGER captured_opportunity_duplicate_suggestions_immutable_delete BEFORE DELETE ON captured_opportunity_duplicate_suggestions BEGIN SELECT RAISE(ABORT, 'captured opportunity duplicate suggestions are immutable'); END;
