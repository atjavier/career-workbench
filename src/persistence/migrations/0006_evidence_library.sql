CREATE TABLE evidence_library_imports (
  id TEXT PRIMARY KEY,
  category TEXT NOT NULL CHECK (category IN ('project', 'experience', 'refresh')),
  ...
  library_root TEXT NOT NULL CHECK (library_root = 'resume-evidence'),
  created_at TEXT NOT NULL,
  UNIQUE (source_identity, source_digest)
);
CREATE TABLE evidence_library_documents (
  id TEXT PRIMARY KEY,
  import_id TEXT REFERENCES evidence_library_imports(id),
  category TEXT NOT NULL CHECK (category IN ('project', 'experience')),
  library_path TEXT NOT NULL CHECK (library_path NOT GLOB '/*' AND library_path NOT GLOB '[A-Za-z]:*' AND library_path NOT GLOB '*..*' AND library_path LIKE 'resume-evidence/%'),
...
CREATE TRIGGER evidence_library_imports_immutable_update BEFORE UPDATE ON evidence_library_imports BEGIN SELECT RAISE(ABORT, 'library imports are immutable'); END;
CREATE TRIGGER evidence_library_imports_immutable_delete BEFORE DELETE ON evidence_library_imports BEGIN SELECT RAISE(ABORT, 'library imports are immutable'); END;
