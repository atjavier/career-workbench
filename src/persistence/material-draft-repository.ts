...
export function insertMaterialDraft(
  db: DatabaseSync,
  item: MaterialDraftRow,
): void {
  db.prepare(
    "INSERT INTO material_drafts (id, kind, profile_revision_id, profile_content_digest, template_source_id, template_content_digest, opportunity_revision_id, opportunity_content_digest, request_text, request_digest, content_json, content_digest, provenance_digest, created_at) VALUES (?, 'resume', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
  ).run(
    item.id,
    ...
    item.templateId,
    item.templateDigest,
    item.opportunityRevisionId ?? null,
    item.opportunityDigest ?? null,
    item.requestText,
    item.requestDigest,
    item.contentJson,
    item.contentDigest,
...
export function insertMaterialDraftEvidence(
  db: DatabaseSync,
  draftId: string,
...
export function insertMaterialDraftClaim(
  db: DatabaseSync,
  item: {
    id: string;
    draftId: string;
  ...
  db.prepare(
    "INSERT INTO material_draft_claims (id, draft_id, ordinal, claim_text, created_at) VALUES (?, ?, ?, ?, ?)",
  ).run(item.id, item.draftId, item.ordinal, item.text, item.createdAt);
}
export function insertMaterialClaimSupport(
  db: DatabaseSync,
  claimId: string,
  evidenceRevisionId: string,
): void {
...
export function findLatestWorkspaceMaterialDraftId(
  db: DatabaseSync,
  workspaceId: string,
): string | undefined {
  const row = db
    .prepare(
      "SELECT d.id FROM material_drafts d JOIN resume_workspace_drafts w ON w.draft_id = d.id WHERE w.workspace_id = ? ORDER BY d.created_at DESC, d.id DESC LIMIT 1",
    )
    .get(workspaceId) as { id: string } | undefined;
  return row?.id;
}

export function isLatestWorkspaceMaterialDraftCurrent(
  db: DatabaseSync,
  input: {
    workspaceId: string;
    profileRevisionId: string;
  ...
  try {
    const parsed = JSON.parse(draft.contentJson) as {
      sections?: Array<{ heading?: unknown }>;
    };
    const headings = new Set(
      (parsed.sections ?? []).map((section) =>
        typeof section?.heading === "string"
          ? section.heading.trim().toLocaleLowerCase()
          : "",
      ),
    );
    ...
    if (
      !headings.has("projects") &&
      !headings.has("project") &&
      !headings.has("experience")
    )
      return false;
  ...
  const actual = (
    db
      .prepare(
        "SELECT evidence_revision_id AS id FROM material_draft_evidence WHERE draft_id = ? ORDER BY evidence_revision_id",
      )
      .all(draft.id) as Array<{ id: string }>
  ).map((item) => item.id);
  return (
...
export function findMaterialDraftRead(
  ...
  const row = db
    ...
    | {
        ...
        opportunityDigest: string | null;
        contentJson: string;
        contentDigest: string;
        provenanceDigest: string;
        templateFilename: string;
        opportunityResolved: string | null;
  ...
  const evidence = (
    db
      .prepare(
        "SELECT e.id, e.content_digest AS contentDigest, e.source_document AS sourceDocument, e.source_section AS sourceSection FROM material_draft_evidence d JOIN evidence_revisions e ON e.id = d.evidence_revision_id WHERE d.draft_id = ? ORDER BY e.source_document, e.source_section, e.id",
      )
    ...
    label: `Documented finding: ${item.sourceDocument} — ${item.sourceSection}`,
  }));
  const claims = (
    db
      .prepare(
        "SELECT id, ordinal, claim_text AS text FROM material_draft_claims WHERE draft_id = ? ORDER BY ordinal",
      )
      .all(draftId) as Array<{ id: string; ordinal: number; text: string }>
  ).map((claim) => ({
    ...claim,
    evidence: (
      db
        .prepare(
          "SELECT e.id, e.source_document AS sourceDocument, e.source_section AS sourceSection FROM material_claim_support s JOIN evidence_revisions e ON e.id = s.evidence_revision_id WHERE s.claim_id = ? ORDER BY e.source_document, e.source_section, e.id",
        )
        .all(claim.id) as Array<{
        id: string;
        sourceDocument: string;
        sourceSection: string;
      }>
  ...
  return {
    ...
    candidateProfileJson: row.candidateProfileJson ?? undefined,
    templateId: row.templateId,
    templateDigest: row.templateDigest,
    opportunityRevisionId: row.opportunityRevisionId ?? undefined,
    opportunityDigest: row.opportunityDigest ?? undefined,
    opportunityResolved: row.opportunityResolved !== null,
    contentJson: row.contentJson,
    contentDigest: row.contentDigest,
    provenanceDigest: row.provenanceDigest,
    templateFilename: row.templateFilename,
    opportunityLabel: row.opportunityLabel ?? undefined,
    handedOffAt: row.handedOffAt ?? undefined,
    evidence,
    claims,
  };
}

export function insertMaterialDraftHandoff(
  db: DatabaseSync,
  item: { id: string; draftId: string; createdAt: string },
): void {
  db.prepare(
    "INSERT INTO material_draft_handoffs (id, draft_id, destination, created_at) VALUES (?, ?, 'review', ?)",
  ).run(item.id, item.draftId, item.createdAt);
}
