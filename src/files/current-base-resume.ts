...
function sourceStorageSegments(
  source: CurrentBaseResumeSource,
): string[] | undefined {
  if (!safeFilename(source.filename) || isAbsolute(source.storageLocation))
    return undefined;
...
export async function readRetainedCurrentBaseResumePdf(
  root: string,
  source: CurrentBaseResumeSource,
  ...
  if (
    [storageRoot, sourceDirectory, filePath].some(
      (path) =>
    ...
    return undefined;
  if (
    !(await isRealDirectory(storageRoot)) ||
    !(await isRealDirectory(sourceDirectory))
  )
    return undefined;
...
export async function stageCurrentBaseResume(
  ...
  try {
    ...
    return {
      ...
      staging,
      final,
      storageLocation: `current-base-resumes/${id}/${filename}`,
    };
  } catch (error) {
...
export async function cleanupCurrentBaseResume(
