...
export type CopiedLibraryContent = {
  documents: MarkdownDocument[];
  sourceDigest: string;
  cleanup: () => Promise<void>;
};
export type DocumentedArtifactSet = CopiedLibraryContent & {
  name: string;
  category: LibraryCategory;
};
export type ResumeDocumentationSource = {
  files: Array<{ path: string; text: string; contentDigest: string }>;
  sourceDigest: string;
};
type UploadedSourceManifest = {
  root: string;
...
function documentationSnapshotPriority(path: string): number {
  ...
  if (
    /^(?:package\.json|pyproject\.toml|cargo\.toml|composer\.json|go\.mod|pom\.xml|build\.gradle(?:\.kts)?|requirements(?:\.txt)?|docker-compose(?:\.ya?ml)?|dockerfile)$/.test(
      normalized,
    )
  )
    return 0;
  ...
  if (
    ...
    return 2;
  if (
    /(?:^|\/)(?:routes?|controllers?|handlers?|api|services?|models?|schemas?|database|db)(?:\/|\.)/.test(
      normalized,
    )
  )
    return 3;
  if (
    ...
    return 4;
  if (
    /(?:^|\/)(?:docs?|documentation|\.github\/workflows)(?:\/|\.)/.test(
      normalized,
    )
  )
    return 5;
  ...
  if (
    /(?:^|\/)(?:eslint|vite|webpack|babel|tsconfig|prettier)\b/.test(normalized)
  )
    return 9;
  return 7;
}
function documentationTraversalPriority(name: string): number {
  ...
  if (/^(?:client|frontend|web|server|backend|api|app|src)$/.test(normalized))
    return 0;
  if (
    /^(?:routes?|controllers?|handlers?|services?|models?|schemas?|entities|database|db|components?|pages?|views?|ui)$/.test(
      normalized,
    )
  )
    return 1;
...
function safeSegment(value: string): string {
  ...
  if (
    ...
    result === "." ||
    result === ".." ||
    reservedWindowsNames.has(result.toUpperCase())
  )
    throw new WorkspaceError(
      "EVIDENCE_LIBRARY_INVALID",
      "The library name needs a safe, non-empty value.",
      "Use a short name containing letters, numbers, spaces, dots, underscores, or hyphens.",
    );
  return result;
}
...
async function requireDirectory(
  ...
  try {
    ...
    throw new WorkspaceError(
      "EVIDENCE_LIBRARY_INVALID",
      "The selected project folder is unavailable or unsafe.",
      nextAction,
    );
  }
}
  ...
  for (let current = resolve(path); ; current = resolve(current, "..")) {
    ...
    if (
      ...
      throw new WorkspaceError(
        ...
        "The selected folder contains an unsafe link or entry.",
        "Choose a working folder without links and try again.",
      );
    const parent = resolve(current, "..");
    if (parent === current) return;
  }
}
async function bytesAndText(
  path: string,
  size: number,
  expected?: Pick<Stats, "dev" | "ino">,
): Promise<{ bytes: Uint8Array; text: string }> {
  if (!size || size > maxDocumentBytes)
    throw new WorkspaceError(
      "EVIDENCE_LIBRARY_INVALID",
      "A selected Markdown document is empty or too large.",
      "Choose Markdown files up to 2 MB and try again.",
    );
  let handle;
  try {
    ...
    if (
      ...
      throw new WorkspaceError(
        "EVIDENCE_LIBRARY_INVALID",
        "A selected Markdown document changed while it was being checked.",
        "Choose the project folder again and try the import.",
      );
    const bytes = await handle.readFile();
    if (bytes.byteLength !== size)
      throw new WorkspaceError(
        "EVIDENCE_LIBRARY_INVALID",
        "A selected Markdown document changed while it was being read.",
        "Choose the project folder again and try the import.",
      );
    try {
      ...
      throw new WorkspaceError(
        "EVIDENCE_LIBRARY_INVALID",
        "A selected Markdown document is not valid UTF-8 text.",
        "Save the document as UTF-8 Markdown and try again.",
      );
    }
  } catch (error) {
    ...
    throw new WorkspaceError(
      "EVIDENCE_LIBRARY_INVALID",
      "A selected Markdown document is unavailable or unsafe.",
      "Choose a readable Markdown document and try again.",
    );
  } finally {
    await handle?.close();
  }
}
export async function enumerateMarkdown(
  root: string,
  category: LibraryCategory,
  allowEmpty = false,
  ...
  await requireDirectory(
    root,
    "Choose an accessible project folder without links and try again.",
  );
  const rootPath = resolve(root);
  const found: MarkdownDocument[] = [];
  async function visit(current: string): Promise<void> {
    for (const entry of await readdir(current, { withFileTypes: true })) {
      ...
      if (metadata.isSymbolicLink())
        throw new WorkspaceError(
          "EVIDENCE_LIBRARY_INVALID",
          "The selected folder contains a link or reparse point.",
          "Choose a folder without links and try again.",
        );
      if (metadata.isDirectory()) await visit(child);
      else if (metadata.isFile() && child.toLowerCase().endsWith(".md")) {
        if (found.length >= maxDocumentsPerImport)
          throw new WorkspaceError(
            "EVIDENCE_LIBRARY_INVALID",
            "The selected folder has too many Markdown documents.",
            "Split the folder into smaller project imports and try again.",
          );
        const captured = await bytesAndText(child, metadata.size, metadata);
        found.push({
          absolutePath: child,
          libraryPath: relative(rootPath, child).replaceAll("\\", "/"),
          ...captured,
          contentDigest: digest(captured.bytes),
          category,
        });
      }
    }
  ...
  if (!found.length && !allowEmpty)
    throw new WorkspaceError(
      "EVIDENCE_LIBRARY_INVALID",
      "No readable Markdown documents were found in the selected folder.",
      "Choose a folder containing Markdown resume evidence and try again.",
    );
  return found;
}
async function cleanManaged(path: string, root: string): Promise<void> {
  if (inside(root, path)) await rm(path, { recursive: true, force: true });
}
export async function copyProjectMarkdown(
  sourceDirectory: string,
  projectName?: string,
  workspaceRoot?: string,
): Promise<CopiedLibraryContent & { projectName: string }> {
  const source = resolve(sourceDirectory);
  const projectNameSafe = safeSegment(projectName || basename(source));
  const root = evidenceLibraryRoot(workspaceRoot);
  const destination = join(root, "projects", projectNameSafe);
  if (source === root || inside(root, source))
    throw new WorkspaceError(
      "EVIDENCE_LIBRARY_INVALID",
      "A managed library folder cannot be added as a project source.",
      "Choose the original project folder outside resume-evidence and try again.",
    );
  if (!inside(root, destination))
    throw new WorkspaceError(
      "EVIDENCE_LIBRARY_INVALID",
      "The project import location is unsafe.",
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === "EEXIST")
      throw new WorkspaceError(
        ...
        "That project has already been added to the evidence library.",
        "Refresh the library or choose a different project name.",
      );
    throw new WorkspaceError(
      "EVIDENCE_LIBRARY_INVALID",
      "The project could not be copied into the evidence library.",
      "Check local folder access and try again.",
    );
  }
  try {
    const documents: MarkdownDocument[] = [];
    for (const item of sourceDocuments) {
      const target = join(destination, item.libraryPath);
      if (!inside(destination, target))
        throw new WorkspaceError(
          "EVIDENCE_LIBRARY_INVALID",
          "A selected document path is unsafe.",
          "Choose a folder without unsafe paths and try again.",
        );
    ...
    return {
      ...
      sourceDigest: digest(
        sourceDocuments
          .map((item) => `${item.libraryPath}:${item.contentDigest}`)
          .sort()
          .join("\n"),
      ),
    ...
    await cleanManaged(destination, root);
    throw error;
  }
}
export async function copyExperienceMarkdown(input: {
  name: string;
  markdown?: string;
  sourceFile?: string;
  workspaceRoot?: string;
}): Promise<CopiedLibraryContent> {
  const name = safeSegment(input.name);
  const root = evidenceLibraryRoot(input.workspaceRoot);
  const destination = join(root, "experiences", `${name}.md`);
  if (!inside(root, destination))
    throw new WorkspaceError(
      "EVIDENCE_LIBRARY_INVALID",
      "The experience import location is unsafe.",
      "Choose a different experience name and try again.",
    );
  ...
  if (input.sourceFile) {
    ...
    captured = await bytesAndText(source, metadata.size, metadata);
  } else {
    ...
    if (!text.trim())
      throw new WorkspaceError(
        ...
        "Enter Markdown content or choose a Markdown file and try again.",
      );
    const bytes = new TextEncoder().encode(text);
    if (bytes.byteLength > maxDocumentBytes)
      throw new WorkspaceError(
        "EVIDENCE_LIBRARY_INVALID",
        "Experience Markdown is too large.",
        "Enter Markdown content up to 2 MB and try again.",
      );
    captured = { bytes, text };
  }
  try {
    await mkdir(join(root, "experiences"), { recursive: true });
    await writeFile(destination, captured.bytes, { flag: "wx" });
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code === "EEXIST")
      throw new WorkspaceError(
        "EVIDENCE_LIBRARY_DUPLICATE",
        "That experience has already been added to the evidence library.",
        "Refresh the library or choose a different experience name.",
      );
    throw new WorkspaceError(
      "EVIDENCE_LIBRARY_INVALID",
      "The experience could not be copied into the evidence library.",
      "Check local folder access and try again.",
    );
  }
  const document = {
  ...
  return {
    documents: [document],
    sourceDigest: digest(captured.bytes),
    cleanup: () => cleanManaged(destination, root),
  };
}
...
const supportingDocumentationArtifacts = [
  ...
  "collaboration-and-process.md",
] as const;
const managedClarificationPacket = "resume-clarifications.md";
const temporaryManagedClarificationPacket =
  /^\.clarifications-[0-9a-f]{8}-[0-9a-f]{4}-7[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\.tmp$/;
type DocumentationArtifactReadContext = "external-output" | "managed-item";
function isCandidateProvidedClarificationPacket(text: string): boolean {
  return /^# Resume Clarifications \(Candidate-Provided\)\r?(?:\n|$)/.test(
    text,
  );
}
function requiredArtifactsFor(category: LibraryCategory): readonly string[] {
  return requiredDocumentationArtifacts[category];
}
function allowedArtifactsFor(category: LibraryCategory): Set<string> {
  return new Set([
    ...requiredArtifactsFor(category),
    ...supportingDocumentationArtifacts,
    ...(category === "experience" ? ["project-overview.md"] : []),
  ]);
}
function hasRequiredArtifacts(
  ...
  const overviewPresent =
    category === "experience"
      ? names.includes("experience-overview.md") ||
        names.includes("project-overview.md")
      : names.includes("project-overview.md");
  return (
    overviewPresent &&
    requiredArtifactsFor(category)
      .filter((name) => !name.endsWith("-overview.md"))
      .every((name) => names.includes(name))
  );
}
const generatedFileName = (name: string) =>
  /(?:^|[._-])(?:generated|autogen|auto-generated)(?:[._-]|$)|\.min\.(?:js|css)$/i.test(
    name,
  );
async function readDocumentationArtifacts(
  directory: string,
  category: LibraryCategory,
  libraryPrefix: string,
  context: DocumentationArtifactReadContext = "external-output",
): Promise<MarkdownDocument[]> {
  await requireDirectory(
    directory,
    "Choose an accessible generated-document folder without links and try again.",
  );
  let entries;
  try {
    ...
    throw new WorkspaceError(
      "EVIDENCE_LIBRARY_INVALID",
      "The generated-document folder is unavailable or unsafe.",
      "Choose the completed documentation output folder and try again.",
    );
  }
  const names = entries.map((entry) => entry.name).sort();
  const allowed = allowedArtifactsFor(category);
  const isTemporaryPacket = (name: string) =>
    context === "managed-item" &&
    temporaryManagedClarificationPacket.test(name);
  if (context === "managed-item") allowed.add(managedClarificationPacket);
  if (
    !hasRequiredArtifacts(category, names) ||
    names.some((name) => !allowed.has(name) && !isTemporaryPacket(name))
  )
    throw new WorkspaceError(
      "EVIDENCE_LIBRARY_INVALID",
      `The generated ${category} documentation folder is missing a required review document or contains an unsupported file.`,
      `Choose the output folder containing the ${category} review documents and supported documentation set.`,
    );
  const documents: MarkdownDocument[] = [];
  for (const name of names) {
    ...
    if (!metadata?.isFile() || metadata.isSymbolicLink())
      throw new WorkspaceError(
        "EVIDENCE_LIBRARY_INVALID",
        "A generated review document is unavailable or unsafe.",
    ...
    if (name === managedClarificationPacket) {
      if (!isCandidateProvidedClarificationPacket(captured.text))
        throw new WorkspaceError(
          "EVIDENCE_LIBRARY_INVALID",
          "The managed clarification packet is malformed.",
    ...
    documents.push({
      absolutePath: path,
      libraryPath: `${libraryPrefix}/${name}`,
      ...captured,
      contentDigest: digest(captured.bytes),
      category,
    });
...
export async function copyDocumentedEvidenceArtifacts(input: {
  outputDirectory: string;
  name: string;
  category: LibraryCategory;
  workspaceId: string;
  workspaceRoot?: string;
}): Promise<DocumentedArtifactSet> {
  const name = safeSegment(input.name);
  const workspaceId = safeSegment(input.workspaceId);
  const source = resolve(input.outputDirectory);
  const root = evidenceLibraryRoot(input.workspaceRoot);
  const categoryRoot =
    input.category === "project" ? "projects" : "experiences";
  const destination = join(root, "workspaces", workspaceId, categoryRoot, name);
  if (source === root || inside(root, source))
    throw new WorkspaceError(
      "EVIDENCE_LIBRARY_INVALID",
      "Generated documents must be outside the managed evidence library.",
      "Choose the separate output folder created by the documentation skill.",
    );
  if (!inside(root, destination))
    throw new WorkspaceError(
      "EVIDENCE_LIBRARY_INVALID",
      "The managed import location is unsafe.",
      "Choose a different item name and try again.",
    );
  const sourceDocuments = await readDocumentationArtifacts(
    source,
    input.category,
    `resume-evidence/workspaces/${workspaceId}/${categoryRoot}/${name}`,
  );
  try {
    await mkdir(join(root, "workspaces", workspaceId, categoryRoot), {
      recursive: true,
    });
    await mkdir(destination);
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code === "EEXIST")
      throw new WorkspaceError(
        ...
        "Choose a different item name or review the existing collection item.",
      );
    throw new WorkspaceError(
      "EVIDENCE_LIBRARY_INVALID",
      "The generated documents could not be copied into the managed library.",
      "Check local folder access and try again.",
    );
  }
  try {
    const documents: MarkdownDocument[] = [];
    for (const item of sourceDocuments) {
      const target = join(destination, item.libraryPath.split("/").at(-1)!);
      await writeFile(target, item.bytes, { flag: "wx" });
      documents.push({ ...item, absolutePath: target });
    }
    return {
      name,
...
export async function readManagedDocumentedArtifacts(
  workspaceRoot?: string,
): Promise<
  Array<{
    name: string;
...
> {
  ...
  for (const workspace of workspaceEntries) {
    ...
    for (const [category, directoryName] of [
      ["project", "projects"],
      ...
      if (!categoryInfo.isDirectory() || categoryInfo.isSymbolicLink())
        throw new WorkspaceError(
          "EVIDENCE_LIBRARY_INVALID",
          "A managed collection folder is unavailable or unsafe.",
          "Check the managed review documents and refresh the page.",
        );
      for (const entry of await readdir(categoryDirectory, {
        withFileTypes: true,
      })) {
        const itemDirectory = join(categoryDirectory, entry.name);
        const itemInfo = await lstat(itemDirectory);
        if (itemInfo.isSymbolicLink())
          throw new WorkspaceError(
            "EVIDENCE_LIBRARY_INVALID",
            "A managed collection item is unsafe.",
            "Check the managed review documents and refresh the page.",
          );
        if (!itemInfo.isDirectory()) continue; // Historical Markdown imports are intentionally outside the active collection.
        const names = (await readdir(itemDirectory)).sort();
        const hasArtifactName = hasRequiredArtifacts(category, names);
        if (!hasArtifactName) continue; // Historical Markdown imports are intentionally outside the active collection.
        groups.push({
          name: entry.name,
          category,
          documents: await readDocumentationArtifacts(
            itemDirectory,
            category,
            `resume-evidence/workspaces/${workspace.name}/${directoryName}/${entry.name}`,
            "managed-item",
          ),
        });
      }
...
export async function readResumeDocumentationSource(
  ...
  if (source === root || inside(root, source))
    throw new WorkspaceError(
      ...
      "Choose the original working folder outside resume-evidence.",
    );
  await requireDirectory(
    source,
    "Choose an accessible working folder without links and try again.",
  );
  const skill = getResumeAgentSkill("resume.document-source-folder");
  const extensions = new Set(skill.allowedExtensions);
  const excludedDirectories = new Set(skill.excludedDirectories);
  ...
  while (
    pending.length &&
    directories < skill.limits.maxDirectories &&
    entriesSeen < skill.limits.maxEntries
  ) {
    const current = pending.shift()!;
    if (current.depth > skill.limits.maxDepth) continue;
    directories += 1;
    await assertSafeAncestors(current.path);
    const entries = (await readdir(current.path, { withFileTypes: true })).sort(
      (left, right) =>
        documentationTraversalPriority(left.name) -
          documentationTraversalPriority(right.name) ||
    ...
    for (const entry of entries) {
      ...
      if (info.isSymbolicLink())
        throw new WorkspaceError(
          "EVIDENCE_DOCUMENTER_INVALID",
          "The selected source folder contains a link or reparse point.",
          "Choose a working folder without links and try again.",
        );
      if (info.isDirectory()) {
      ...
      if (
        !info.isFile() ||
        generatedFileName(entry.name) ||
        !extensions.has(extname(entry.name).toLowerCase()) ||
        info.size === 0 ||
        info.size > skill.limits.maxFileBytes
      )
        continue;
      candidates.push({
        absolutePath: path,
  ...
  for (const candidate of candidates.sort(
    (left, right) =>
      documentationSnapshotPriority(left.path) -
        documentationSnapshotPriority(right.path) ||
    ...
    if (
      JSON.stringify([...files, file]).length <= skill.limits.maxSnapshotChars
    )
      files.push(file);
  }
  if (!files.length)
    throw new WorkspaceError(
      "EVIDENCE_DOCUMENTER_INVALID",
      "No safe text material was found in the selected source folder.",
      "Choose a working folder containing readable documentation or source files.",
    );
...
function invalidSnapshot(
  message: string,
  nextAction = "Choose the folder again and try documentation.",
): never {
  throw new WorkspaceError("EVIDENCE_DOCUMENTER_INVALID", message, nextAction);
}
function snapshotPath(value: string): string {
  if (
    ...
    invalidSnapshot(
      "The selected folder snapshot has an unsafe relative path.",
    );
...
export async function readUploadedResumeDocumentationSource(input: {
  files: File[];
  manifest: string;
}): Promise<ResumeDocumentationSource> {
  if (input.manifest.length > 64_000)
    invalidSnapshot(
      "The selected folder details exceed the safe inspection boundary.",
  ...
  try {
    ...
    invalidSnapshot("The selected folder details are unavailable.");
  }
  if (
    !manifest ||
    typeof manifest !== "object" ||
    !Array.isArray((manifest as UploadedSourceManifest).files) ||
    !(manifest as UploadedSourceManifest).files.length ||
    (manifest as UploadedSourceManifest).files.length !== input.files.length ||
    typeof (manifest as UploadedSourceManifest).root !== "string"
  )
    invalidSnapshot("The selected folder details do not match its files.");
  const skill = getResumeAgentSkill("resume.document-source-folder");
  const extensions = new Set(skill.allowedExtensions);
  const excluded = new Set(skill.excludedDirectories);
  const paths = new Set<string>();
  const directories = new Set<string>();
  const files: ResumeDocumentationSource["files"] = [];
  const root = snapshotPath((manifest as UploadedSourceManifest).root);
  if (root.includes("/"))
    invalidSnapshot("The selected folder details do not match its files.");
  if (input.files.length > skill.limits.maxFiles)
    invalidSnapshot(
      "The selected folder has too many eligible files.",
      "Choose a smaller folder and try documentation.",
    );
  for (const [index, file] of input.files.entries()) {
    ...
    if (
      ...
      invalidSnapshot("The selected folder details do not match its files.");
    const submittedPath = snapshotPath(item.path);
    const submittedParts = submittedPath.split("/");
    if (submittedParts[0] !== root || submittedParts.length < 2)
      invalidSnapshot(
        "The selected folder details do not describe one folder.",
      );
    const path = submittedParts.slice(1).join("/");
    const parts = path.split("/");
    if (
      parts.at(-1) !== file.name ||
      paths.has(path) ||
      parts.length - 1 > skill.limits.maxDepth ||
      parts.slice(0, -1).some((part) => excluded.has(part.toLowerCase())) ||
      generatedFileName(file.name) ||
      !extensions.has(extname(file.name).toLowerCase()) ||
      !file.size ||
      file.size > skill.limits.maxFileBytes
    )
      invalidSnapshot(
        "The selected folder contains unsupported or unsafe files.",
        "Choose a smaller folder with supported source files and try documentation.",
      );
    paths.add(path);
    for (let depth = 1; depth < parts.length; depth += 1)
      directories.add(parts.slice(0, depth).join("/"));
    if (
      directories.size > skill.limits.maxDirectories ||
      input.files.length + directories.size > skill.limits.maxEntries
    )
      invalidSnapshot(
        "The selected folder exceeds the safe inspection boundary.",
        "Choose a smaller folder and try documentation.",
      );
    ...
    if (
      JSON.stringify([...files, candidate]).length >
      skill.limits.maxSnapshotChars
    )
      invalidSnapshot(
        "The selected folder exceeds the safe inspection boundary.",
        "Choose a smaller folder and try documentation.",
...
export async function readManagedMarkdown(
  workspaceRoot?: string,
): Promise<MarkdownDocument[]> {
  const root = evidenceLibraryRoot(workspaceRoot);
  try {
    await requireDirectory(
      root,
      "Add a project or experience before refreshing the library.",
    );
  } catch {
    throw new WorkspaceError(
      "EVIDENCE_LIBRARY_EMPTY",
      "The Resume Evidence Library is empty.",
      "Add a project or experience, then refresh the library.",
    );
  }
  ...
  for (const [category, name] of [
    ...
    const categoryRoot = join(root, name);
    try {
      await lstat(categoryRoot);
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code === "ENOENT") continue;
      throw new WorkspaceError(
        "EVIDENCE_LIBRARY_INVALID",
        "A managed library folder is unavailable.",
        "Check the Resume Evidence Library folder and refresh again.",
      );
    }
    const documents = await enumerateMarkdown(categoryRoot, category, true);
    const documentedParents = new Set<string>();
    for (const document of documents) {
      const parent = resolve(document.absolutePath, "..");
      if (document.libraryPath.split("/").length === 2) {
        const names = (await readdir(parent)).sort();
        if (
          hasRequiredArtifacts(category, names) &&
          names.every((entry) => allowedArtifactsFor(category).has(entry))
        )
          documentedParents.add(parent);
      }
    }
    result.push(
      ...documents.filter(
        (document) =>
          !documentedParents.has(resolve(document.absolutePath, "..")),
      ),
    );
  }
  if (!result.length)
    throw new WorkspaceError(
      "EVIDENCE_LIBRARY_EMPTY",
      "The Resume Evidence Library has no Markdown documents.",
      "Add a project or experience, then refresh the library.",
    );
