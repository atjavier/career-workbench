...
export function validateResumeDraftContent(value: unknown): ResumeDraftContent {
  if (!value || typeof value !== "object")
    throw new WorkspaceError(
      ...
      "The Current Base Resume draft is unavailable.",
      "Refresh the Current Base Resume and try again.",
    );
  const candidate = value as Record<string, unknown>;
  const keys: Array<keyof ResumeDraftContent> = [
    "contact",
    "summary",
    "experience",
    "projects",
  ...
  for (const key of keys) {
    ...
    if (
      ...
      throw new WorkspaceError(
        "CURRENT_BASE_RESUME_INVALID",
        "The Current Base Resume draft contains unsupported text.",
        "Review the draft text and try again.",
      );
    length += lines.reduce((total, line) => total + line.length, 0);
    if (length > maximumCurrentResumeText)
      throw new WorkspaceError(
        "CURRENT_BASE_RESUME_INVALID",
...
export function structureResumeText(text: string): ResumeDraftContent {
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    ...
    if (!line) continue;
    const found = sections.find(([, heading]) =>
      heading.test(line.replace(/:$/, "")),
    );
    if (found) {
      section = found[0];
      continue;
...
export async function parseResumePdf(
  ...
  if (
    bytes.byteLength === 0 ||
    bytes.byteLength > maximumCurrentResumeBytes ||
    bytes[0] !== 0x25 ||
    bytes[1] !== 0x50 ||
    bytes[2] !== 0x44 ||
    bytes[3] !== 0x46
    ...
    throw new WorkspaceError(
      "CURRENT_BASE_RESUME_INVALID",
      "The selected file is not a readable resume PDF.",
      "Choose a text-readable PDF up to 10 MB and try again.",
    );
  }
  ...
  try {
    ...
    if (document.numPages < 1 || document.numPages > maximumCurrentResumePages)
      throw new WorkspaceError(
        "CURRENT_BASE_RESUME_INVALID",
        "The selected PDF cannot be used as a resume source.",
        "Choose a text-readable PDF with no more than 12 pages and try again.",
      );
    const lines: string[] = [];
    let used = 0;
    for (let pageNumber = 1; pageNumber <= document.numPages; pageNumber += 1) {
      ...
      try {
        ...
        const line = pageLines
          ...
          .join("\n");
        used += line.length;
        if (used > maximumCurrentResumeText)
          throw new WorkspaceError(
            "CURRENT_BASE_RESUME_INVALID",
            "The selected PDF contains too much text to import safely.",
            "Choose a shorter text-readable resume PDF and try again.",
          );
    ...
    if (!text)
      throw new WorkspaceError(
        ...
        "Choose a text-readable PDF; scanned images are not supported yet.",
      );
