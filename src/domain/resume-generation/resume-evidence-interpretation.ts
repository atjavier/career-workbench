...
const projectTaskQuestions: Record<TaskCategory, (name: string) => string> = {
  ...
  ownership: (name) =>
    `What parts of ${name} did you personally build, design, or contribute?`,
  users_workflow: (name) =>
    `Who was ${name} for, and what workflow did it support?`,
  outcome: (name) => `What capability or change did ${name} deliver?`,
  metrics: (name) =>
    `Are there any measured results, scale details, or evaluation results for ${name}?`,
  deployment: (name) => `What was the deployment or usage status of ${name}?`,
  collaboration: (name) =>
    `Did you work with anyone on ${name}, and what was your role?`,
...
const experienceTaskQuestions: Record<TaskCategory, (name: string) => string> =
  {
    purpose: (name) =>
      `What was the primary focus, product, or mission of your team at ${name}?`,
    ownership: (name) =>
      `What systems, features, or workflows did you personally build or contribute to at ${name}?`,
    users_workflow: (name) =>
      `Who used the systems or tools you worked on at ${name}, and what workflows did they support?`,
    outcome: (name) =>
      `What key deliverables, improvements, or outcomes resulted from your work at ${name}?`,
    metrics: (name) =>
      `Are there any measured results, scale details, or performance metrics from your work at ${name}?`,
    deployment: (name) =>
      `What was the release, deployment, or production status of the systems you worked on at ${name}?`,
    collaboration: (name) =>
      `Who did you collaborate with at ${name}, and what was your role on the team?`,
...
export function planClarifications(
  ...
  if (
    !has(
      facts,
      /\b(purpose|problem|intended to|designed to address|address|objective)\b/i,
    )
  )
    planned.push("purpose");
  if (
    !has(facts, /\b(i |my role|personally|responsible for|contributed to)\b/i)
  )
    planned.push("ownership");
  if (
    !has(facts, /\b(user|users|researcher|student|client|workflow|used by)\b/i)
  )
    planned.push("users_workflow");
  if (
    !has(
      facts,
      /\b(enabled|delivered|result|outcome|evaluation|improved|streamlined)\b/i,
    )
  )
    planned.push("outcome");
  if (!has(facts, /\b\d+\b|percent|respondents|score|ms|seconds|users\b/i))
    planned.push("metrics");
  if (
    !has(
      facts,
      /\b(deployed|deployment|production|staging|released|demo(?: mode)?|used by)\b/i,
    )
  )
    planned.push("deployment");
  if (!has(facts, /\b(team|collaborat|adviser|advisor|group)\b/i))
    planned.push("collaboration");
  if (!has(facts, /\b20\d{2}\b|semester|ay\s*20/i)) planned.push("dates");
  if (
    !has(facts, /\b(role|developer|researcher|student|intern|contributor)\b/i)
  )
    planned.push("role");
  const questionMap =
    category === "experience" ? experienceTaskQuestions : projectTaskQuestions;
  return planned.map((cat) => ({
    category: cat,
    question: questionMap[cat](name),
  }));
}
...
export async function interpretWorkspaceEvidence(
  ...
  options: { appDataRoot?: string; workspaceRoot?: string } = {},
): Promise<boolean> {
