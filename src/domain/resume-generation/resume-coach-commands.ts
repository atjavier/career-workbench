...
export function persistResumeCoachDraft(input: PersistResumeCoachDraftInput): { id: string } {
  ...
  try { applyMigrations(db); db.exec("BEGIN IMMEDIATE;"); try {
    const activeWorkspace = readActiveResumeWorkspace(db).workspace; if (!activeWorkspace || activeWorkspace.id !== input.workspaceId) throw new WorkspaceError("RESUME_WORKSPACE_STALE", "Your resume workspace changed before its base resume could be saved.", "Refresh Resume and let the current workspace generate its preview again."); const contentJson = JSON.stringify(input.response); const requestDigest = digest(input.requestText); const contentDigest = digest(contentJson); const provenanceDigest = digest({ capability: localModelCapabilityVersion("resume-coach"), profile: { revisionId: input.profileRevisionId, contentDigest: input.profileDigest }, template: { id: input.templateId, contentDigest: input.templateDigest }, evidence: input.evidence.map(({ id, contentDigest }) => ({ id, contentDigest })).sort((left, right) => left.id.localeCompare(right.id)), opportunity: input.opportunity ? { revisionId: input.opportunity.revisionId, contentDigest: input.opportunity.contentDigest } : null, consent: input.consentFingerprint });
    ...
    for (const evidence of input.evidence) insertMaterialDraftEvidence(db, id, evidence.id); attachDraftToWorkspace(db, input.workspaceId, id);
