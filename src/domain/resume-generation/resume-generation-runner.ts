...
import {
  readResumeGenerationJob,
  updateResumeGenerationJob,
} from "@/domain/resume-generation/resume-generation-jobs";
...
export type ResumeGenerationWorkItem = {
  category: "project" | "experience";
  name: string;
  sourceDirectory: string;
};

async function workspaceHasDocumentedEvidence(
  workspaceId: string,
): Promise<boolean> {
  ...
  try {
    ...
    return listWorkspaceDocumentedEvidenceIds(database, workspaceId).length > 0;
  } finally {
    database.close();
  }
}
...
export async function runResumeGenerationJob(
  jobId: string,
  work: ResumeGenerationWorkItem[],
): Promise<void> {
  try {
    ...
    if (!job) return;
    await updateResumeGenerationJob(
      jobId,
      "running",
      "Reading your selected local work folders and creating documentation.",
    );
    for (const item of work) {
      ...
      try {
        ...
        if (
          ...
          (await workspaceHasDocumentedEvidence(job.workspaceId))
        )
          continue;
        throw error;
      }
    ...
    await updateResumeGenerationJob(
      jobId,
      "running",
      "Creating and typesetting your resume PDF.",
    );
    const formData = new FormData();
    formData.set("workspaceId", job.workspaceId);
    const generated = await generateBaseResumeAction(
      { status: "idle", summary: "" },
      formData,
    );
    if (generated.status !== "success" || !generated.draftId)
      throw new WorkspaceError(
        "RESUME_COACH_INVALID",
        generated.summary || "Your resume could not be generated.",
        "Review your local AI setup and documented work, then try again.",
      );
    await compileResumeDraftPdf(
      await readMaterialDraft({ draftId: generated.draftId }),
    );
    await updateResumeGenerationJob(
      jobId,
      "completed",
    ...
    await updateResumeGenerationJob(jobId, "failed", safe.summary).catch(
      () => undefined,
    );
  } finally {
    ...
    try {
      ...
      /* The next page visit will read current durable state. */
    }
  }
}
