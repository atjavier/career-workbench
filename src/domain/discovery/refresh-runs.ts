...
import { adapterKey, sourceAdapterRegistry, type SourceAdapter } from "@/adapters/sources/source-adapter";
...
import { finalizeRefreshRun, insertRefreshRun, insertRefreshSourceOutcome, listRefreshRuns as listRefreshRunsRepository, type StoredRefreshSourceOutcome } from "@/persistence/refresh-runs-repository";
...
export type RefreshSourceOutcome = Omit<StoredRefreshSourceOutcome, "contentDigest">;
...
function toOutcome(source: Candidate, refreshRunId: string, status: RefreshSourceOutcome["status"], requestCount: number, recoveryGuidance: string, startedAt: string): RefreshSourceOutcome { return { id: createUuidV7(), refreshRunId, sourceId: source.sourceId, sourceConfigurationRevisionId: source.revisionId, status, requestCount, recoveryGuidance, startedAt, completedAt: new Date().toISOString() }; }
...
export async function listRefreshRuns(options: Options = {}): Promise<RefreshRunsView> {
  return withDatabase(options, false, (database) => {
    ...
    const runs = listRefreshRunsRepository(database).map((run) => ({ id: run.id, status: run.status, selectedSourceCount: run.selectedSourceCount, startedAt: run.startedAt, completedAt: run.completedAt ?? run.startedAt, outcomes: run.outcomes.map(({ contentDigest: _digest, ...outcome }) => outcome) }));
...
export async function startRefreshRun(input: Options & { sourceIds: string[]; confirmed: boolean }): Promise<RefreshRun> {
  ...
  for (const source of candidates) {
    ...
    try {
      ...
      const result = await adapter.refresh({ source, signal, consumeRequest });
      const status = result.httpStatus === 429 ? "throttled" : result.httpStatus === 401 || result.httpStatus === 403 ? "blocked" : result.status;
      outcomes.push(toOutcome(source, runId, status, requestCount, result.recoveryGuidance ?? source.values.failureGuidance, startedAt));
    } catch (error) {
      const status = error instanceof WorkspaceError && error.code === "REFRESH_POLICY_UNRESOLVED" ? "throttled" : "failed";
      outcomes.push(toOutcome(source, runId, status, requestCount, error instanceof WorkspaceError ? error.safeNextAction : failedGuidance, startedAt));
    }
  }
  const status = runStatus(outcomes); const completedAt = new Date().toISOString();
  await withDatabase(input, true, (database) => { for (const outcome of outcomes) insertRefreshSourceOutcome(database, { ...outcome, contentDigest: digest({ refreshRunId: outcome.refreshRunId, sourceId: outcome.sourceId, revisionId: outcome.sourceConfigurationRevisionId, status: outcome.status, requestCount: outcome.requestCount }) }); finalizeRefreshRun(database, runId, status, completedAt); appendAuditEvent(database, createAuditEvent({ actor: "local-os-user", action: status === "failed" ? "discovery.refresh_failed" : "discovery.refresh_finished", outcome: status === "failed" ? "failure" : "success", entityId: runId, contentHash: digest({ runId, status }) })); });
