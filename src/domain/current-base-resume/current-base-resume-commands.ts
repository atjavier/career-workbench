...
function legacyWritesAreHistoryOnly(): never { throw new WorkspaceError("CURRENT_BASE_RESUME_HISTORY_ONLY", "Current Base Resume is retained as read-only history.", "Use the Profile-led Resume workspace for new resume details."); }
...
export async function saveCurrentBaseResumeDraft(_input: Options & { draftId: string; content: ResumeDraftContent }): Promise<CurrentBaseResumeDraft> { return legacyWritesAreHistoryOnly(); }
export async function generateCurrentBaseResumeProposals(_input: Options & { draftId: string }): Promise<CurrentBaseResumeProposal[]> { return legacyWritesAreHistoryOnly(); }
export async function resolveCurrentBaseResumeProposal(_input: Options & { proposalId: string; expectedDecisionRevisionId: string; decision: "approved" | "edited" | "rejected"; text?: string }): Promise<void> { return legacyWritesAreHistoryOnly(); }
export async function approveCurrentBaseResumeVersion(_input: Options & { draftId: string; explicitApproval: boolean }): Promise<CurrentBaseResumeVersion> { return legacyWritesAreHistoryOnly(); }
