...
export class OpportunityCaptureValidationError extends WorkspaceError {
  ...
  constructor(
    field: OpportunityCaptureValidationError["field"],
    summary: string,
...
function normalizePostingUrl(value: unknown): string {
  if (typeof value !== "string" || !value.trim())
    throw new OpportunityCaptureValidationError(
      ...
      "Paste the HTTPS link from the page you found.",
    );
  const raw = value.trim();
  if (raw.length > maximumUrlLength)
    throw new OpportunityCaptureValidationError(
      "postingUrl",
      "The posting URL is too long.",
      "Use the direct HTTPS link, up to 2,048 characters.",
    );
  try {
    ...
    if (
      ...
      parsed.username ||
      parsed.password ||
      parsed.toString().length > maximumUrlLength
    )
    ...
    throw new OpportunityCaptureValidationError(
      "postingUrl",
      "Enter a valid HTTPS posting URL without embedded credentials.",
...
function normalizeDescription(value: unknown): string {
  if (typeof value !== "string")
    throw new OpportunityCaptureValidationError(
      "copiedDescription",
      "Paste the copied job description.",
      "Copy the role details from the page, then try again.",
    );
  ...
  if (text.length > maximumDescriptionLength)
    throw new OpportunityCaptureValidationError(
      "copiedDescription",
      "The copied job description is too long.",
      "Shorten the copied text to 200,000 characters or fewer.",
    );
  return text;
}
...
function requirementsFrom(text: string): string[] {
  const section = text.match(
    /(?:^|\n)\s*(?:requirements?|qualifications?|what you(?:'|’)ll need)\s*:?\s*\n([\s\S]*?)(?=\n\s*\n|\n\s*(?:benefits?|about|responsibilities|equal opportunity)\b|$)/i,
  )?.[1];
  const entries =
    section
      ?.split("\n")
...
function titleFrom(text: string): string {
  const labelledTitle = labelledValue(text, [
    "job title",
    "title",
    "position",
    "role",
  ...
  return /^(?:about(?: the role| us)?|job description|overview|responsibilities|requirements?|qualifications?|benefits?|company|employer|location|work style|work arrangement|posted(?: date| on)?|date posted)\s*:?$/i.test(
    firstLine,
  )
    ? unknown
    : firstLine;
...
export function captureOpportunityDraft(
  input: OpportunityCaptureInput,
