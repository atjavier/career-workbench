...
import { latestCapturedOpportunityAssessment, latestCapturedOpportunityDecision, type OpportunityAssessmentView, type OpportunityDecisionView } from "@/domain/fit/ai-opportunity-assessment";
import { latestCapturedFitAssessmentView, type FitFactor } from "@/domain/fit/fit-assessment";
...
export default async function Home() {
  ...
  const assessmentState = await Promise.all(opportunitiesState.opportunities.map(async (opportunity) => [opportunity.id, await latestCapturedOpportunityAssessment(opportunity.id).catch(() => undefined), await latestCapturedOpportunityDecision(opportunity.id).catch(() => undefined), await (async () => { const paths = await resolveAppDataPaths(); const db = openDatabase(paths.databasePath); try { applyMigrations(db); return latestCapturedFitAssessmentView(db, opportunity.id); } finally { db.close(); } })().catch(() => undefined)] as const));
  const assessments = Object.fromEntries(assessmentState.map(([id, assessment, decision, fit]) => [id, { assessment, latestDecision: decision, fit }])) as Record<string, { assessment?: OpportunityAssessmentView; latestDecision?: OpportunityDecisionView; fit?: { label: "Strong" | "Potential" | "Stretch"; confidence: "high" | "medium" | "low"; calculatedAt: string; factors: FitFactor[] } }>;

  return <ApplicationShell active="Jobs"><div className="workspace-shell jobs-page-shell">
    <JobListings opportunities={opportunitiesState.opportunities} materials={materials} assessments={assessments} error={opportunitiesState.error} />
    <WorkspaceStatus />
