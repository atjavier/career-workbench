...
export default async function Home() {
  ...
  const assessmentState = await Promise.all(opportunitiesState.opportunities.map(async (opportunity) => [opportunity.id, await latestCapturedOpportunityAssessment(opportunity.id).catch(() => undefined), await latestCapturedOpportunityDecision(opportunity.id).catch(() => undefined)] as const));
  const assessments = Object.fromEntries(assessmentState.map(([id, assessment, decision]) => [id, { assessment, latestDecision: decision }])) as Record<string, { assessment?: OpportunityAssessmentView; latestDecision?: OpportunityDecisionView }>;
