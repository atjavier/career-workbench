...
export function MaterialDraftReview({ draft }: { draft: MaterialDraftView }) {
  return <div className="workspace-shell material-draft-review">
    ...
    <section className="material-draft-review-content" aria-labelledby="review-claims-heading"><h2 id="review-claims-heading">Grounded claims</h2><ul>{draft.claims.map((claim, index) => <li key={`${claim.text}-${index}`}><p>{claim.text}</p>{claim.evidence.length ? <p><strong>Supported by approved evidence:</strong> {claim.evidence.join("; ")}</p> : null}{claim.candidateClarifications.length ? <p><strong>Candidate-provided context:</strong> {claim.candidateClarifications.map((clarification) => `${clarification.itemName} (${clarification.category}): ${clarification.text}`).join("; ")}</p> : null}</li>)}</ul></section>
