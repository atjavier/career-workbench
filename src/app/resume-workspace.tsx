...
export async function ResumeWorkspace() {
  ...
  if (!workspaceState.activeWorkspace && workspaceState.workspaces.length === 0) return <div className="workspace-shell resume-workspace"><header className="resume-page-head"><div className="resume-head-copy"><p className="eyebrow">Resume</p><h1>Start your resume</h1><p>Save your basic information and choose your local work folders to document. You can add more folders anytime.</p></div><ResumeWorkspacePicker workspaces={workspaceState.workspaces} activeWorkspaceId={undefined} revisionNumber={workspaceState.revisionNumber} /></header><ResumeOnboarding localAiReady={localModel.ready} /></div>;
  return <div className="workspace-shell resume-workspace"><header className="resume-page-head"><div className="resume-head-copy"><p className="eyebrow">Resume</p><h1 aria-label="Shape your base resume">Shape your base resume</h1><p>Resume Coach is on the left and your automatically generated base-resume preview is on the right. Manage <Link href="/evidence">Experience &amp; Projects</Link> separately.</p></div><ResumeWorkspacePicker workspaces={workspaceState.workspaces} activeWorkspaceId={workspaceState.activeWorkspace?.id} revisionNumber={workspaceState.revisionNumber} /></header><section id="resume-edit" aria-label="Resume Edit"><ResumeCoach available={Boolean(workspaceState.activeWorkspace && localModel.ready && !("error" in profile) && profile.revision && (materials.length || generationJob?.status === "queued" || generationJob?.status === "running"))} unavailableReason={missing} showSetupLink={aiMissing} initialDraft={draftState.draft} generationNeeded={!draftState.current && generationJob?.status !== "queued" && generationJob?.status !== "running"} generationMessage={generationJob?.status === "queued" || generationJob?.status === "running" ? generationJob.message : generationJob?.status === "failed" ? generationJob.message : undefined} workspaceId={workspaceState.activeWorkspace?.id} /></section></div>;
}
