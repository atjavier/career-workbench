...
export function OpportunityCapture({ open, onClose, onReturnToAllOpportunities = onClose }: { open: boolean; onClose: () => void; onReturnToAllOpportunities?: () => void }) {
  ...
  return <dialog ref={dialogRef} id="opportunity-capture" className="opportunity-capture" aria-labelledby="opportunity-capture-heading" aria-describedby="opportunity-capture-description" onCancel={(event) => { if (formPending || confirmationPendingRef.current || captureActionLocked) event.preventDefault(); }} onClose={onClose}>
    ...
    {draftIsCurrent && state.draft ? <OpportunityConfirmation key={state.draft.capturedAt} draft={state.draft} onPendingChange={setConfirmationStatus} onReturnToAllOpportunities={() => { onReturnToAllOpportunities(); requestAnimationFrame(() => document.getElementById("job-listings")?.scrollIntoView({ block: "start" })); }} /> : null}
  </dialog>;
}

function OpportunityConfirmation({ draft, onPendingChange, onReturnToAllOpportunities }: { draft: NonNullable<OpportunityCaptureActionState["draft"]>; onPendingChange: (pending: boolean) => void; onReturnToAllOpportunities: () => void }) {
