...
export async function GET(_request: Request, { params }: { params: Promise<{ revisionId: string }> }): Promise<Response> {
  try {
    ...
    return new Response(responseBody(draft.tex), { headers: { "cache-control": "private, no-store", "content-disposition": "attachment; filename=editable-resume.tex", "content-length": String(draft.tex.byteLength), "content-type": "application/x-tex; charset=utf-8", "cross-origin-resource-policy": "same-origin", "x-content-type-options": "nosniff" } });
  } catch { return new Response("The editable TeX draft is unavailable.", { status: 404, headers: unavailableHeaders }); }
}
