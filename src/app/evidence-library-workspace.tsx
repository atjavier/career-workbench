...
export async function EvidenceLibraryWorkspace() {
  const [collection, workspaceState] = await Promise.all([
    listExperienceProjectCollection().then((items) => ({ items, error: undefined })).catch((error) => ({ items: [], error: safeError(error) })),
    readResumeWorkspaceState().catch(() => ({ workspaces: [], activeWorkspace: undefined, revisionNumber: 0 })),
  ]);
  return <div className="workspace-shell resume-workspace experience-projects-workspace"><header className="resume-page-head"><div className="resume-head-copy"><p className="eyebrow">Resume</p><h1>Experience &amp; Projects</h1><p>Keep source folders separate from the reviewable evidence behind your resume.</p></div><div className="resume-view-tabs sr-only" aria-hidden="true" /><ResumeWorkspacePicker workspaces={workspaceState.workspaces} activeWorkspaceId={workspaceState.activeWorkspace?.id} revisionNumber={workspaceState.revisionNumber} /></header><section id="experience-projects" aria-label="Experience and Projects"><EvidenceLibrary collection={collection.items} error={collection.error} /></section></div>;
}
