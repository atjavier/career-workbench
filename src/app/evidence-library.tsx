...
export function EvidenceLibrary({
  ...
  return (
    ...
    >
      <div className="experience-projects-intro">
        <p className="eyebrow">Your work</p>
        <h2 id="experience-projects-heading">Projects and experiences</h2>
        <p>
          Give your local AI one real working folder. It reads a bounded,
          allowlisted local snapshot directly, returns proposed documents, and
      ...
      <section
        ...
        role="tabpanel"
        aria-labelledby={
          category === "project" ? "projects-tab" : "experiences-tab"
        }
      >
        <div className="experience-project-actions">
          <button
            type="button"
          ...
          <p>
            One consented local-AI documentation run makes source-grounded
            findings available to Resume Coach.
          </p>
        </div>
        {open ? (
          <section
            className="documentation-handoff panel"
            aria-labelledby="documentation-handoff-heading"
          >
            <h3 id="documentation-handoff-heading">
              Document this {label.toLowerCase()}
            </h3>
            <p>
              The selected folder may contain source code, tests, docs,
              manifests, configuration, assets, and generated material. It is
              not uploaded. The agent performs a bounded local inspection,
              writes no source files, and keeps unsupported content as unknown.
            </p>
            <p>
              It creates{" "}
              <code>
              ...
              , <code>resume-evidence.md</code>, and{" "}
              <code>resume-bullet-candidates.md</code>. Every current documented
            ...
            <form
              ...
              className="generated-document-import"
            >
              <input
                type="hidden"
                name="libraryCommand"
              ...
              <label htmlFor="documented-item-name">{label} name</label>
              <input
                id="documented-item-name"
                name="itemName"
                required
              ...
              >
                <span>{selectedFolder?.name ?? "No folder selected"}</span>
                <button
                  type="button"
              ...
              <label>
                <input
                  ...
                  required
                />{" "}
                I confirm this folder may be inspected by my configured local
                AI.
              </label>
        ...
        {!selected.length ? (
          <div className="collection-empty">
            <h3>No documented {label.toLowerCase()}s yet</h3>
            <p>Start one local documentation run above.</p>
          </div>
        ) : (
          <ul className="experience-project-list">
            {selected.map((item) => {
              ...
              return (
                <li key={key} className="experience-project-row">
                  <div>
                    <h3>{item.name}</h3>
                    <p>{item.summary}</p>
                    <p className="collection-meta">
                      {item.artifactNames.length} documentation files ·{" "}
                      {item.evidence.length} documented finding
                      {item.evidence.length === 1 ? "" : "s"} · Ready to use
                    </p>
                  </div>
                  <button
                    type="button"
                  ...
                  <button
                    ...
                    onClick={() =>
                      setDeleting(deleting === key ? undefined : key)
                    }
                  >
                  ...
                  {deleting === key ? (
                    <form action={action} className="documented-item-delete">
                      ...
                      <label htmlFor={`${key}-delete-confirmation`}>
                        Type DELETE to permanently remove this {item.category},
                        its managed documentation, and its documented findings.
                        Your original folder is not changed.
                      </label>
                      <input
                        id={`${key}-delete-confirmation`}
                        name="confirmation"
                        required
                      />
                      <button
                  ...
                  {detailsOpen ? (
                    <div
                      id={`${key}-details`}
                      className="experience-project-details"
                    >
                      <p>Documentation set: {item.artifactNames.join(", ")}.</p>
                      {item.evidence.length ? (
                        <ul>
                          {item.evidence.map((evidence) => (
                            <li key={evidence.reviewHandle}>
                              <p>{evidence.factualText}</p>
                              <p>{findingLabel[evidence.reviewState]}</p>
                              <form action={findingAction}>
                                <input
                                  type="hidden"
                                  name="reviewHandle"
                        ...
                        <p>No supported findings were returned.</p>
                      )}
                    </div>
                  ) : null}
                </li>
