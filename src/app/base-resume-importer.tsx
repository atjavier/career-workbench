...
import {
  importBaseResumeAction,
  type WorkspaceActionState,
} from "@/app/actions";

const initialState: WorkspaceActionState = {
  status: "idle",
  summary: "Select a .tex resume and supported companion files to import.",
};
...
export function BaseResumeImporter() {
  ...
  return (
    <section aria-labelledby="base-resume-import-heading" className="panel">
      ...
      <p>
        Choose one .tex resume and optional .pdf, .aux, .log, .out, or
        .synctex.gz companion files. Imported files become read-only.
      </p>
      <form action={action}>
        <label htmlFor="base-resume-files">Base Resume files</label>
        <input
          id="base-resume-files"
          name="baseResumeFiles"
          type="file"
          multiple
          accept=".tex,.pdf,.aux,.log,.out,.synctex.gz"
          aria-invalid={state.status === "error"}
          aria-describedby={
            state.status === "error" ? "base-resume-import-error" : undefined
          }
          onChange={(event) =>
            setFilenames(
        ...
        <button type="submit" disabled={pending}>
          {pending ? "Importing…" : "Import Base Resume"}
        </button>
      </form>
      <p
        id={state.status === "error" ? "base-resume-import-error" : undefined}
